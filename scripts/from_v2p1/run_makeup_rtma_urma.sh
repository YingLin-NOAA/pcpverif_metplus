...
if [ ${run_pcprtma} -eq 1 ]; then
  ...
  export modpath=/gpfs/dell2/ptmp/Ying.Lin/prod_rtma
...
if [ ${run_pcprtma2p8} -eq 1 ]; then
  ...
  export MODEL=`echo $model | tr a-z A-Z`
  export modpath=/gpfs/dell2/ptmp/Ying.Lin/wget_rtma2p8
...
if [ ${run_pcpurma} -eq 1 ]; then
  ...
  export MODEL=`echo $model | tr a-z A-Z`
  export modpath=/gpfs/dell2/ptmp/Ying.Lin/prod_urma
...
if [ ${run_pcpurma2p8} -eq 1 ]; then
  ...
  export MODEL=`echo $model | tr a-z A-Z`
  export modpath=/gpfs/dell2/ptmp/Ying.Lin/wget_urma2p8
